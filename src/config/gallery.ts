...
export interface WorkItem {
   ...
   * Art & Programming") stays visible while THIS tile's banner covers the ART|CODE
   * hero. Omitted/`true` ⇒ retained (default, no change); `false` ⇒ the card fades
   * out (opacity 0 + pointer-events:none) while the banner is on, so the cover reads
   * clean, fading back on hover exit. Per-tile — like {@link bannerFx} — so a wide or
   * text-heavy banner can hide the card while other tiles keep it.
   * See handoff/banner-swap-tagline-retained.md.
   */
  bannerSwapTaglineRetained?: boolean;
  href: string;
